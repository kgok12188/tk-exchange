...
本设计将交易系统的核心路径从 Kafka 迁移到 Aeron，重点是 match-engine 侧的架构。trading-server 的结果共识设计后续补充。

当前架构：

```
open-api → Kafka trading_(shard) → trading-server → Kafka order_req_(symbol)
→ match-engine → Kafka match_result_(symbol) → message-dispatch
→ Kafka trading_(shard) → trading-server → Kafka trading_result_(shard) → flush
```

目标架构：

```
open-api → trading-server (结果共识, 待设计)
→ Aeron Cluster session: match-engine (指令共识)
→ Aeron MDC + Spy Archive: MatchResult
...
- 为 trading-server 和行情 service 定义消费 match-engine 输出的方式。

**Non-Goals:**

- 不设计 trading-server 的结果共识实现（后续补充）。
...
- **ClusteredService 伪逻辑**:
  ...
  onSessionMessage(session, buffer):
      result = engine.process(decode(buffer))
      if (result != null):
          encoded = encode(result)
          localPub.offer(encoded)            // 所有节点 → 本地 Archive spy
          if (cluster.role() == LEADER):
              mdcPub.offer(encoded)           // 仅 Leader → 网络
  ```
...
- **Rationale**:
  ...
  - 与当前 Chronicle Queue 文件队列相比：不需要手动管理 slave/master 双目录、补发逻辑、进程启动清理等。

### 7. ReplayMerge：消费者的统一入口
...
- **工作原理**:
  ...
  3. ReplayMerge 内部同时发起 Archive replay（从 lastProcessedPosition）和 live MDC subscription。
  4. Replay 阶段：从 Archive 回放历史 MatchResult。
  5. Merge 阶段：replay 追上 live 时自动无缝切换。
  6. Live 阶段：直接消费 MDC 实时数据。
  7. 消费者始终使用同一个 `replayMerge.poll(handler, fragmentLimit)` 接口，无需区分阶段。
- **match-engine 切主时的消费者行为**:
  - 因为所有节点维护相同的 Archive（见 §6），消费者可连接任意节点的 Archive。
  - 检测到 live MDC 中断后，重建 ReplayMerge，连接新 Leader 的 Archive 和 MDC。
  - 从 lastProcessedPosition 继续，无数据丢失。
- **Rationale**:
  - 替代当前 Kafka consumer group 的 offset 管理 + 手动 seek + poll 模式。
...
- **onTakeSnapshot(snapshotPublication)**:
  - 遍历所有 `engines` 中的 MatchEngine。
  - 对每个 symbol：编码 `SnapshotHeader`（symbolId, reqOffset, orderCount, nextSeq），然后遍历 OrderBook 的所有挂单编码为 `SnapshotBookOrder`。
  - 通过 `snapshotPublication.offer(buffer)` 写入。
  - 使用 SBE 编码（非 JSON），与 wire 协议一致。
- **onLoadSnapshot(snapshotImage)**:
  - 从 `snapshotImage` 读取并解码 SnapshotHeader + SnapshotBookOrder。
  - 按 seq 升序 restoreOrder 重建每个 symbol 的 OrderBook。
...
### 9. 删除 message-dispatch，match 结果直达 trading-server

- **Decision**: 删除 message-dispatch 独立服务。trading-server 直接通过 ReplayMerge 订阅 match-engine 的 MDC 输出，在内部完成 MatchResult → per-user 拆分和路由。
- **trading-server 内部处理**:
  ...
  - 吸收 `TradingSettleTransformer` 的拆分逻辑（约 40 行代码）。
- **Rationale**:
  - 消除一个独立服务的部署和运维开销。
  - 减少一跳延迟（当前 Kafka match_result → dispatch → Kafka trading_ ≈ 4-10ms）。
  - Aeron MDC 天然支持多订阅者，trading-server 各 shard 和行情 service 可直接订阅，无需中间层。
  - match-engine 不感知 shard 拓扑（只发全量 MatchResult），解耦干净。

### 10. match-engine Ingress：Aeron Cluster Client Sessions
...
- **Rationale**:
  ...
  - 替代当前的 Kafka `order_req_(symbol)` topic（每 symbol 一个 topic + consumer）。

### 11. orderReqOffset 语义变化

- **Decision**: `MatchResult.orderReqOffset` 的语义从 Kafka `order_req_(symbol)` partition offset 变为 **Aeron Cluster log position**。
- **影响**:
  - 值域和单调性不变（Cluster log position 全局单调递增）。
  - 不再 per-symbol（Cluster log 是全局的），但每条 MatchResult 仍然绑定到触发它的那条命令的 log position。
  - 下游幂等逻辑（`(orderReqOffset, index)` 唯一标识一笔成交）不变。
- **Rationale**: 语义一致（"触发本次撮合的命令的位置标识"），只是来源从 Kafka offset 变为 Cluster log position。

### 12. trading-server 结果共识（待设计）

- **Decision**: trading-server 从当前的 "主从都消费 trading_(shard) + 主写 trading_result + 从写 Chronicle" 模式，改为 **结果共识**：仅 Leader 执行命令并产生副作用（发 order_req、发 response、写结果日志），结果复制给 Follower。
- **为什么不用指令共识**:
...
- **Status**: 具体实现方案后续讨论。占位以明确方向。

## Risks / Trade-offs

- **[Risk] Aeron 运维经验不如 Kafka 成熟**
...
- **[Risk] match-engine 切主时 Archive 连续性**
  **Mitigation**: 所有节点（Leader + Follower）都维护完整的本地 Archive（因为都执行相同命令 → 确定性相同结果）。切主后消费者连接新 Leader 的 Archive，从 lastProcessedPosition 继续，无数据丢失。
